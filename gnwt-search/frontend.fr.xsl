...
<!-- *** THIS FILE HAS BEEN EDITED BY GNWT EMPLOYEE CURTIS NAPHAN. 
	 *** EDIT HISTORY
		...
		2014-02-06	CURTIS NAPHAN	Forked from English version
...
<xsl:variable name="search_button_text">Recherche</xsl:variable>
<xsl:variable name="search_prompt">Que cherchez-vous?</xsl:variable>
...
<xsl:variable name="sep_bar_adv_text">Recherche avancée</xsl:variable>
<xsl:variable name="sep_bar_error_text">Erreur</xsl:variable>
...
<xsl:variable name="spelling_text">Essayez avec l'orthographe:</xsl:variable>
...
<xsl:variable name="synonyms_text">Vous pouvez également essayez:</xsl:variable>
...
<xsl:variable name="default_publisher">Gouvernement des Territoires du Nord-Ouest</xsl:variable>
...
<xsl:template name="my_page_header">
      ...
      <div class="container">
        <div class="navbar">
          <ul>
			...
			<li><a href="http://www.gov.nt.ca/research/departments/">Ministères</a> </li>
			<li><a href="http://services.exec.gov.nt.ca/">Services</a> </li>
			<li><a href="http://www.gov.nt.ca/utility/contact/index.html">Nous joindre</a> </li>
	...
	<header>
      <div class="container">
        <a href="http://www.gov.nt.ca/" title="Logo du Gouvernment des Territoires du Nord-Ouest"><img class="logo pull-left" src="http://www.fin.gov.nt.ca/theme/consistent-user-experience-1.1/common/images/gnwt-logo-fr.png"/></a>
        <h1 class="name single-row"><a title="Recherche" href="http://google.gov.nt.ca">Recherche</a></h1>
      </div>
    </header> <!--/header -->
        ...
        <div class="container">
          <div class="navbar">
            <ul class="nav navbar-nav">
              <li><a href="http://rdirectory.gov.nt.ca/rDirectory.aspx">Annuaire</a></li>
              <li><a href="http://www.gov.nt.ca/utility/contact/index.html">Nous joindre</a></li>
              <li><a href="http://www.gov.nt.ca/terms/">Conditions d'utilisation</a></li>
              <li><a href="http://www.executive.gov.nt.ca/accessibility/">Accessibilité</a></li>
              <li><a href="http://www.gov.nt.ca/active.htm">Offre active</a></li>
              <li><a href="http://www.gov.nt.ca/utility/contact/index.html">Salle de presse</a></li>
            </ul>
          </div>
        </div>        
      </div>
...
<span class="p">
	<br/>
	Aucun document ne correspond aux termes de recherche spécifiés (<b><xsl:value-of select="$query"/></b>).
	<br/>
	<br/>
	Suggestions :
	<ul>
...
<xsl:variable name="url_all_collections"><xsl:value-of select="$gsa_search_root_path_prefix"/>?<xsl:for-each
      select="/GSP/PARAM[(@name != 'start') and
						 (@name != 'site') and
                         (@name != $embedded_mode_root_path_param) and
                         (@name != $embedded_mode_resource_root_path_param) and
  ...
  <xsl:if test="$render_dynamic_navigation = '1'">
    <div class="dn-bar">
      ...
      <a class="dn-link" style="text-decoration: underline; color: #000;"
        href="{$gsa_search_root_path_prefix}?{$all_results_url}">Tous resultats</a>
	...
	<span id="service_{$res_num}" class="cue-service-descriptor">
		<xsl:choose>
			...
			<xsl:when test="MT[@N='dcterms.type']/@V='Form'">
				<img src="http://www.fin.gov.nt.ca/images/search/form.png" width="16" height="16" alt="Formulaire"/> Formulaire
			</xsl:when>
			...
			<xsl:when test="MT[@N='dcterms.type']/@V='News'">
				<img src="http://www.fin.gov.nt.ca/images/search/news.png" width="16" height="16" alt="Nouvelle"/> Nouvelle: <xsl:value-of select="FS[@NAME='date']/@VALUE"/>
			</xsl:when>
			<xsl:otherwise>
		...
		<xsl:if test="MT[@N='dcterms.type']">
			...
			<xsl:if test="MT[@N='dcterms.publisher'] and MT[@N='dcterms.publisher']/@V != $default_publisher">
				<span id="publisher={$res_num}" class="cue-publisher">
					Publié par: <value-of select="MT[@N='dcterms.publisher']/@V"/>
				</span>
			</xsl:if>
		</xsl:if>
			<!-- *** URL *** -->
    ...
    <table border="0" cellpadding="0" width="1%" cellspacing="0">
      <tr align="center" valign="top">
        ...
        <td valign="bottom" nowrap="1">
          <font size="-1">
            Page de resultat<xsl:call-template name="nbsp"/>
          </font>
        </td>
        </xsl:if>
...
<xsl:template name="language_link">
	<xsl:variable name="frontend">test_frontend</xsl:variable>
	<xsl:variable name="other_lang">lang_en</xsl:variable>
	...
	<xsl:text disable-output-escaping="yes">&amp;getfields=*"&gt;English&lt;/a&gt;</xsl:text>
</xsl:template>	 
