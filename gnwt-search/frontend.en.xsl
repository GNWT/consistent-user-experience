...
<!-- *** THIS FILE HAS BEEN EDITED BY GNWT EMPLOYEE CURTIS NAPHAN. 
	 *** EDIT HISTORY
		DATE		NAME			CHANGE SUMMARY
		2013-12-08	CURTIS NAPHAN	Created file for first time. Stripped down configuration to minimum. Added metadata markup.
		2014-02-27	CURTIS NAPHAN	Made responsive on smaller screens. But suggestions and embedded mode no longer work.
	 *** -->
...
<xsl:variable name="logo_url">http://www.fin.gov.nt.ca/images/nwt_logo.png</xsl:variable>
<xsl:variable name="logo_width">82</xsl:variable>
<xsl:variable name="logo_height">71</xsl:variable>
...
<xsl:variable name="search_prompt">What are you looking for?</xsl:variable>
...
<xsl:variable name="show_sep_bar">0</xsl:variable>
<xsl:variable name="choose_sep_bar">nothing</xsl:variable>
<xsl:variable name="sep_bar_std_text"></xsl:variable> <!-- Search -->
...
<!-- *** synonyms suggestions - Related Queries *** -->
...
<xsl:variable name="egds_appliance_tab_label">Enterprise Search</xsl:variable>
...
<xsl:variable name="analytics_account">UA-44701725-3</xsl:variable>
...
<xsl:variable name="dyn_nav_max_rows">12</xsl:variable>
...
<!-- EDIT CN - Show suggestions is broken. Very hard to make render correctly on mobile, as is table-based -->
...
<!-- EDIT CN *** CUE variables *** -->
<xsl:variable name="default_publisher">Government of the Northwest Territories</xsl:variable>
<xsl:variable name="show_favicon">1</xsl:variable>
<xsl:variable name="favicon_url">http://www.fin.gov.nt.ca/theme/consistent-user-experience-1.1/common/images/favicon.ico</xsl:variable>
<xsl:variable name="all_collections_site">All_GNWT</xsl:variable>
...
<xsl:template name="my_page_header">
    <div class="topnav">
      <div class="container">
        <div class="navbar">
          <ul>
            <li class="first"><a href="http://www.gov.nt.ca">gov.nt.ca</a></li>
			<li><a href="http://www.gov.nt.ca/research/departments/">Departments</a> </li>
			<li><a href="http://services.exec.gov.nt.ca/">Services</a> </li>
			<li><a href="http://www.gov.nt.ca/utility/contact/index.html">Contact</a> </li>
			<li><xsl:call-template name="language_link"/></li>
          </ul>
        </div>
      </div>
    </div><!--/.topnav -->
	...
	<header>
      <div class="container">
        <a href="http://www.gov.nt.ca/" title="Logo of the Government of the Northwest Territories"><img class="logo pull-left" src="http://www.fin.gov.nt.ca/theme/consistent-user-experience-1.1/common/images/gnwt-logo.png"/></a>
        <h1 class="name single-row"><a title="Home" href="http://google.gov.nt.ca">Search</a></h1>
      </div>
    </header> <!--/header -->
        ...
        <div class="container">
          <div class="navbar">
            <ul class="nav navbar-nav">
              <li class="first leaf"><a href="http://rdirectory.gov.nt.ca/rDirectory.aspx">Directory</a></li>
              <li class="leaf"><a href="http://www.gov.nt.ca/utility/contact/index.html">Contact</a></li>
              <li class="leaf"><a href="http://www.gov.nt.ca/terms/">Terms of Use</a></li>
              <li class="leaf"><a href="http://www.executive.gov.nt.ca/accessibility/">Accessibility</a></li>
              <li class="leaf"><a href="http://www.gov.nt.ca/active.htm">Active Offer</a></li>
              <li class="last leaf"><a href="http://www.gov.nt.ca/utility/contact/index.html">Newsroom</a></li>
            </ul>
          </div>
        </div>        
      </div> <!-- .navbar -->
...
<xsl:variable name="sep_bar_bg_color">
  <xsl:choose>
    ...
    <xsl:when test="$choose_sep_bar = 'ltblue'">#DBDBDB</xsl:when>
    <xsl:when test="$choose_sep_bar = 'blue'">#3366cc</xsl:when>
    <xsl:otherwise><xsl:value-of select="$global_bg_color"/></xsl:otherwise>
  </xsl:choose>
</xsl:variable>
...
.b,.b a{font-size: 12pt; font-weight:bold}
...
<!-- *** EDIT CN url_all_collections: standard URL but with site set to all_collections *** -->
<xsl:variable name="url_all_collections">
	<xsl:value-of select="$gsa_search_root_path_prefix"/>?<xsl:for-each
      select="/GSP/PARAM[@name != 'site']">
    <xsl:choose>
      <xsl:when test="@name = 'only_apps' and $show_apps_segmented_ui = '1'">
        <xsl:value-of select="'exclude_apps=1'" />
      </xsl:when>
  ...
  <xsl:text disable-output-escaping='yes'>&amp;site=</xsl:text>
  <xsl:value-of select="$all_collections_site"/>
</xsl:variable>
...
<select name="site">
 <xsl:choose>
  <xsl:when test="PARAM[(@name='site') and (@value='AAIR')]">
   <option value="AAIR" selected="selected">AAIR</option>
  </xsl:when>
  <xsl:otherwise>
 ...
 <xsl:choose>
  <xsl:when test="PARAM[(@name='site') and (@value='All_GNWT')]">
   <option value="All_GNWT" selected="selected">All_GNWT</option>
  </xsl:when>
  <xsl:otherwise>
   <option value="All_GNWT">All_GNWT</option>
  </xsl:otherwise>
 </xsl:choose>
 <xsl:choose>
  <xsl:when test="PARAM[(@name='site') and (@value='ENR')]">
   <option value="ENR" selected="selected">ENR</option>
  </xsl:when>
  <xsl:otherwise>
 ...
 <xsl:choose>
  <xsl:when test="PARAM[(@name='site') and (@value='Education')]">
 ...
 <xsl:choose>
  <xsl:when test="PARAM[(@name='site') and (@value='FAMWeb')]">
   <option value="FAMWeb" selected="selected">FAMWeb</option>
  </xsl:when>
  <xsl:otherwise>
   <option value="FAMWeb">FAMWeb</option>
  </xsl:otherwise>
 </xsl:choose>
 <xsl:choose>
 ...
 <xsl:choose>
  <xsl:when test="PARAM[(@name='site') and (@value='HR_Manager')]">
   <option value="HR_Manager" selected="selected">HR_Manager</option>
  </xsl:when>
  <xsl:otherwise>
 ...
 <xsl:choose>
  <xsl:when test="PARAM[(@name='site') and (@value='HR_Manual')]">
   <option value="HR_Manual" selected="selected">HR_Manual</option>
  </xsl:when>
  <xsl:otherwise>
   <option value="HR_Manual">HR_Manual</option>
  </xsl:otherwise>
 </xsl:choose>
 <xsl:choose>
  <xsl:when test="PARAM[(@name='site') and (@value='Health')]">
 ...
 <xsl:choose>
  <xsl:when test="PARAM[(@name='site') and (@value='Housing_Corp')]">
   <option value="Housing_Corp" selected="selected">Housing_Corp</option>
  </xsl:when>
  <xsl:otherwise>
   <option value="Housing_Corp">Housing_Corp</option>
  </xsl:otherwise>
 </xsl:choose>
 <xsl:choose>
  <xsl:when test="PARAM[(@name='site') and (@value='Human_Resources')]">
   <option value="Human_Resources" selected="selected">Human_Resources</option>
  </xsl:when>
  <xsl:otherwise>
 ...
 <xsl:choose>
  <xsl:when test="PARAM[(@name='site') and (@value='ITI')]">
   <option value="ITI" selected="selected">ITI</option>
  </xsl:when>
  <xsl:otherwise>
 ...
 <xsl:choose>
  <xsl:when test="PARAM[(@name='site') and (@value='Justice')]">
   <option value="Justice" selected="selected">Justice</option>
  </xsl:when>
  <xsl:otherwise>
 ...
 <xsl:choose>
  <xsl:when test="PARAM[(@name='site') and (@value='Legislative_Assembly')]">
   <option value="Legislative_Assembly" selected="selected">Legislative_Assembly</option>
  </xsl:when>
  <xsl:otherwise>
   <option value="Legislative_Assembly">Legislative_Assembly</option>
  </xsl:otherwise>
 </xsl:choose>
 <xsl:choose>
  <xsl:when test="PARAM[(@name='site') and (@value='MACA')]">
   <option value="MACA" selected="selected">MACA</option>
  </xsl:when>
  <xsl:otherwise>
 ...
 <xsl:choose>
  <xsl:when test="PARAM[(@name='site') and (@value='Public_Works')]">
   <option value="Public_Works" selected="selected">Public_Works</option>
  </xsl:when>
  <xsl:otherwise>
 ...
 <xsl:choose>
  <xsl:when test="PARAM[(@name='site') and (@value='TestAllGNWT')]">
   <option value="TestAllGNWT" selected="selected">TestAllGNWT</option>
  </xsl:when>
  <xsl:otherwise>
   <option value="TestAllGNWT">TestAllGNWT</option>
  </xsl:otherwise>
 </xsl:choose>
 <xsl:choose>
  <xsl:when test="PARAM[(@name='site') and (@value='Transportation')]">
...
<xsl:template name="search_box">
	...
	<div id="search-prompt">
		<xsl:value-of select="$search_prompt"/>
	</div>
	<xsl:choose>
		<xsl:when test="$show_suggest = '1' and (($type = 'home') or ($type = 'std_top'))">
			<input type="text" class="form-control" name="q" size="{$search_box_size}" maxlength="256" value="{$space_normalized_query}" autocomplete="off" onkeyup="ss_handleKey(event)" />
			<table cellpadding="0" cellspacing="0" class="ss-gac-m" style="width: 365px; visibility: hidden;" id="search_suggest"></table>
		</xsl:when>
		<xsl:otherwise>
			<input type="text" class="form-control" name="q" size="{$search_box_size}" maxlength="256" value="{$space_normalized_query}"/>
		</xsl:otherwise>
	</xsl:choose>
...
<xsl:template name="gen_top_navigation">
  <xsl:if test="RES and ($show_top_navigation != 0 or $show_sort_by != 0)">
    <table width="100%">
      <tr>
        <xsl:if test="$show_top_navigation != '0'">
          <td align="left">
...
<xsl:template name="dynamic_navigation_results">
  ...
  <!-- show sort-by -->
  <xsl:if test="$show_sort_by != '0' or ($show_spelling != '0' and /GSP/Spelling/Suggestion) or ($show_synonyms != '0' and /GSP/Synonyms/OneSynonym)">
...
<xsl:template name="render_main_results">
  ...
  <div class="{$main_results_class}">
    <!-- EDIT CN If using a specific site collection, add a link to search all sites -->
	<xsl:if test="PARAM[@name='site'] and PARAM[@name='site']/@value != $all_collections_site">		
		<div id="cue_search_all">
			<xsl:text disable-output-escaping="yes">&lt;a href="</xsl:text>
			<xsl:value-of select="$url_all_collections"/>
			<xsl:text disable-output-escaping="yes">"&gt;Search all GNWT websites&lt;/a&gt;</xsl:text>			
		</div>
	</xsl:if>
...
<xsl:template match="R">
  ...
  <xsl:if test="$show_res_title != '0'">
    <xsl:choose>
      ...
      <xsl:when test="@MIME='text/plain'"><span class="res-doc-type">[TEXT]</span></xsl:when>
      <xsl:when test="@MIME='application/rtf'"><span class="res-doc-type">[RTF]</span></xsl:when>
      <xsl:when test="@MIME='application/pdf'"><span class="res-doc-type">[PDF]</span></xsl:when>
      <xsl:when test="@MIME='application/postscript'"><span class="res-doc-type">[PS]</span></xsl:when>
      <xsl:when test="@MIME='application/vnd.ms-powerpoint'"><span class="res-doc-type">[MS POWERPOINT]</span></xsl:when>
      <xsl:when test="@MIME='application/vnd.ms-excel'"><span class="res-doc-type">[MS EXCEL]</span></xsl:when>
      <xsl:when test="@MIME='application/msword'"><span class="res-doc-type">[MS WORD]</span></xsl:when>
      <xsl:otherwise>
    ...
    <span id="title_{$res_num}" class="l">
		<xsl:choose>	
		  <!-- EDIT CN Added clause below. If title metadata exists, display it instead of the usual title -->
		  <xsl:when test="MT[@N='dcterms.title']">
			  <xsl:value-of select="MT[@N='dcterms.title']/@V"/>
		  </xsl:when>
		  <xsl:when test="T">
		...
		<!-- EDIT CN If a municipal link, add the municipal icon after the title -->
		<xsl:if test="contains(U, 'yellowknife.ca') or contains(U, 'hayriver.com') or contains(U, 'fortsmith.ca') or contains(U, 'fortmcpherson.ca') or contains(U, 'inuvik.ca') or contains(U, 'tlicho.ca') or contains(U, 'aklavik.ca') or contains(U, 'fortliard.com') or contains(U, 'fortsimpson.com')or contains(U, 'deline.ca') ">
			<img src="http://www.fin.gov.nt.ca/images/search/mun.png" width="16" height="16" alt="Municipal result" class="cue-municipal-icon"/>
		</xsl:if>
    </span>
    <xsl:if test="$link">
  ...
  <!-- *** Result Type *** -->
  <!-- EDIT CN If a type is given, display a type line before the description/snippet -->
  <xsl:if test="MT[@N='dcterms.type']">	 		
	<span id="service_{$res_num}" class="cue-service-descriptor">
		<xsl:choose>
			<xsl:when test="MT[@N='dcterms.type']/@V='Service'">
				<img src="http://www.fin.gov.nt.ca/images/search/gear.png" width="16" height="16" alt="Service"/> Service
			</xsl:when>
			<xsl:when test="MT[@N='dcterms.type']/@V='Form'">
				<img src="http://www.fin.gov.nt.ca/images/search/form.png" width="16" height="16" alt="Form"/> Form
			</xsl:when>
			<xsl:when test="MT[@N='dcterms.type']/@V='Publication'">
				<img src="http://www.fin.gov.nt.ca/images/search/pub.png" width="16" height="16" alt="Publication"/> Publication: <xsl:value-of select="FS[@NAME='date']/@VALUE"/>
			</xsl:when>
			<xsl:when test="MT[@N='dcterms.type']/@V='FAQ'">				
				<img src="http://www.fin.gov.nt.ca/images/search/faq.png" width="16" height="16" alt="FAQ"/> FAQ
			</xsl:when>
			<xsl:when test="MT[@N='dcterms.type']/@V='News'">
				<img src="http://www.fin.gov.nt.ca/images/search/news.png" width="16" height="16" alt="News"/> News: <xsl:value-of select="FS[@NAME='date']/@VALUE"/>
			</xsl:when>
			<xsl:otherwise>
				<img src="http://www.fin.gov.nt.ca/images/search/info.png" width="16" height="16" alt="Information"/> Information
			</xsl:otherwise>		
		</xsl:choose>
	</span>
  </xsl:if>
  ...
  <div class="s">
		<xsl:choose>
			<!-- EDIT CN If dcterms.description is given, show it instead of the Google snippet -->
			<xsl:when test="MT[@N='dcterms.description']">
				<!-- TODO CN If dcterms.description is too long, truncate it -->
				<xsl:value-of select="MT[@N='dcterms.description']/@V"/>
			</xsl:when>
		    <xsl:when test="$show_res_snippet != '0' and string-length(S) and
                      $only_apps != '1'">
		...
		<!-- If the "dcterms.type" metadata is given, don't show the size - date - cached line like normal results -->
		<xsl:if test="MT[@N='dcterms.type']">
			<!-- EDIT CN If this page isn't from the GNWT, show the publisher -->
			<xsl:if test="MT[@N='dcterms.publisher'] and MT[@N='dcterms.publisher']/@V != $default_publisher">
				<span id="publisher={$res_num}" class="cue-publisher">
					Published by: <value-of select="MT[@N='dcterms.publisher']/@V"/>
				</span>
			</xsl:if>
		</xsl:if>
			<!-- *** URL *** -->
			...
			<div id="url_{$res_num}" class="cue-url">
			  <xsl:choose>
...
<script>
  ...
  ga('create', 'UA-44701725-3', 'gov.nt.ca');
  ga('send', 'pageview');
...
<xsl:template name="plainHeadStart">
	...
	<link rel="stylesheet" href="http://www.fin.gov.nt.ca/theme/consistent-user-experience-1.1/common/cue-basic.css"/>  
	<link rel="stylesheet" href="http://www.fin.gov.nt.ca/theme/consistent-user-experience-1.1/common/cue-layout.css"/>  
	<link rel="stylesheet" href="http://www.fin.gov.nt.ca/theme/gnwt-search.css"/>  
	<link rel="stylesheet" type="text/css" media="screen and (max-width: 780px)"	href="http://www.fin.gov.nt.ca/theme/gnwt-search-mobile.css" />
</xsl:template>
...
<!-- **********************************************************************
 EDIT CN Consistent User Experience templates
     ********************************************************************** -->

<!-- Displays the language toggle link -->	 
<xsl:template name="language_link">
	<xsl:variable name="frontend">test_frontend_fr</xsl:variable>
	<xsl:variable name="other_lang">lang_fr</xsl:variable>
	
	<xsl:text disable-output-escaping="yes">&lt;a href="</xsl:text>
	<xsl:value-of select="$gsa_search_root_path_prefix"/>
	<xsl:text disable-output-escaping="yes">?q=</xsl:text>
	<xsl:value-of select="encode-for-uri($qval)"/>
	<xsl:text disable-output-escaping="yes">&amp;site=</xsl:text>
	...
	<xsl:value-of select="$frontend"/>
	<xsl:text disable-output-escaping="yes">&amp;proxystylesheet=</xsl:text>
	<xsl:value-of select="$frontend"/>
	<xsl:text disable-output-escaping="yes">&amp;lr=</xsl:text>
	<xsl:value-of select="$other_lang"/>
	<xsl:text disable-output-escaping="yes">&amp;getfields=*"&gt;Français&lt;/a&gt;</xsl:text>
</xsl:template>	 

<!-- Outputs the favicon tags in the <head> element -->
